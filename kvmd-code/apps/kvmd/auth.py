...
class AuthManager:
    def __init__(
        ...
        get_logger(0).debug("start AuthManager init()")

        self.__enabled = enabled
        if not enabled:
            get_logger(0).warning("AUTHORIZATION IS DISABLED")
        ...
        if enabled:
            ...
            get_logger(0).info("Using internal auth service %r", self.__internal_service.get_plugin_name())
        ...
        if enabled and external_type:
            ...
            get_logger(0).info("Using external auth service %r", self.__external_service.get_plugin_name())

        self.__force_internal_users = force_internal_users

        self.__tokens: Dict[str, str] = {}  # {token: user}
        
        get_logger(0).debug("end AuthManager init()")
    ...
    async def authorize(self, user: str, passwd: str) -> bool:
        ...
        if ok:
            get_logger(0).info("Authorized user %r via auth service %r", user, service.get_plugin_name())
        else:
            get_logger(0).error("Got access denied for user %r from auth service %r", user, service.get_plugin_name())
        return ok
    ...
    async def login(self, user: str, passwd: str) -> Optional[str]:
        ...
        if (await self.authorize(user, passwd)):
            ...
            get_logger(0).info("Logged in user %r", user)
            return token
        else:
            return None
    ...
    def logout(self, token: str) -> None:
        ...
        if user:
            get_logger(0).info("Logged out user %r", user)
