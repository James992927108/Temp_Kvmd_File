...
def init(
    ...
    **load: bool,
    ...
    # get_logger(0).debug("check_run : %r", check_run)
    if check_run:
    ...
    # get_logger(0).debug("options.dump_config : %r", options.dump_config)
    ...
    # get_logger(0).debug("call _init_config()")
    config = _init_config(options.config_path, options.set_options, **load)
    # get_logger(0).debug("finish _init_config()")
...
def _init_config(config_path: str, override_options: List[str], **load_flags: bool) -> Section:
    ...
    try:
        ...
        # get_logger(0).debug("end _init_config()")
        return config
...
def _dump_config(config: Section) -> None:
    # get_logger(0).debug("start _dump_config()")
